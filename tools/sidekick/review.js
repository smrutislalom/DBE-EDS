...
async function getReviewStatus() {
  ...
  if (reviews.length === 1 && reviews[0].reviewId === SNAPSHOT_ID) return reviews[0].status;
  return ('open');
}
...
async function previewMode(plugins, sk) {
  ...
  try {
    ...
    sk.addEventListener('custom:move-to-review', async () => {
      ...
      window.location.href = `https://${SNAPSHOT_ID}--${env.ref}--${env.repo}--${env.owner}.aem.reviews${window.location.pathname}`;
    });
  } catch (e) {
